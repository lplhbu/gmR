function consume(array) {
    const val = array.map(a => a.split('.')[0]);
...
function loadObject(toData, fromData, matches, fromFields, toFields) {
    ...
    for (let i = 0; i < Math.min(fromFields.length, toFields.length); i++) {
        if (!fromFields[i] && !toFields[i]) toData[thisToFields[i]] = fromData[thisFromFields[i]];
        else load(toData[thisToFields[i]], fromData[thisFromFields[i]], [...matches], fromFields ? fromFields.filter(ff => ff) : null, toFields ? toFields.filter(tf => tf) : null)
    }
...
function load(toData, fromData, matches = null, fromFields = null, toFields = null) {
    ...
    if (Array.isArray(toData)) loadArray(toData, fromData, matches, fromFields, toFields);
    else loadObject(toData, fromData, matches, fromFields, toFields);
}
